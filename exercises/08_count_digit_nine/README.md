## Lesson 8 Find 9 in number 1 to 100 1 到 100 有多少个 9
...
- 为什么不写一个函数，直接就能计算出 1-100 中的 9 的个数？

### 课后练习

- 求 1-100 以内最大的素数，要求用设计一个函数实现。
- 用户输入两个数字，按从个位对齐的方式，找出这 2 个数在相同位置处数字也相同的个数。  
  例如：123 和 5173 这 2 个数字，位置相同数字也相同的个数是 2
